defmodule DB.Query.Dynamic do
  alias __MODULE__.Builder

  def build(schema, assoc_map, search_input) do
    %{filter: filter_map, page: page_map, sort: sort_map, opts: opts} =
      search_input

    main_table = schema.__table__()
    main_alias = Map.fetch!(assoc_map, main_table)

    {wheres, where_assocs, _, bindings} =
      Builder.build_wheres(assoc_map, filter_map)
