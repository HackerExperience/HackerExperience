...
defmodule DB.Type.DateTimeUTC do
  def sqlite_type, do: :text
...
defmodule DB.Type.Map do
  ...
  def load!(v, %{keys: :atom}) when is_binary(v),
    do: Jason.decode!(v, keys: :atoms)

  def load!(v, _) when is_binary(v), do: Jason.decode!(v, keys: :atoms)
  def load!(nil, %{nullable: true}), do: nil
end

defmodule DB.Type.List do
  def sqlite_type, do: :text
