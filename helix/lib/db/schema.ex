defmodule DB.Schema do
    ...
    quote do
      ...
      ordered_schema =
        ...
        |> Enum.map(fn {column_name, v} ->
          ...
          type_mod = :"Elixir.DB.Type"
          datatype_module = type_mod.get_module(datatype)

          # Break compilation dependency
          mod_module = :"Elixir.DB.Mod"
    ...
    quote do
      ...
      new_args =
        Enum.map(unquote(args), fn {field, value} ->
          {field, DB.Schema.cast_value!(schema, field, value)}
        end)
  ...
  def validate(%_{} = struct, field) when is_atom(field) do
    ...
    case apply(mod, field, [Map.get(struct, field), []]) do
      {:ok, _} -> :ok
      {:error, _} = e -> e
