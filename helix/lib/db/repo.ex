defmodule DB.Repo do
  ...
  def get_path(context, shard_id), do: "#{@data_dir}/#{context}/#{shard_id}.db"
  ...
  def handle_continue(:bootstrap, state) do
    ...
    if @env == :test do
      ...
      :ok = SQLite.exec(state.conn, "PRAGMA synchronous=0")
    else
      # Use NORMAL synchronous mode, which is safe in WAL[2] mode.
      :ok = SQLite.exec(state.conn, "PRAGMA synchronous=1")
    end
  ...
  def handle_continue(:check_migrations, state) do
    DB.Migrator.get_migration_status(state.conn, state.context, state.mode)

    # TODO: Wrap the migration in a transaction
    case DB.Migrator.get_migration_status(state.conn, state.context, state.mode) do
      :migrated ->
        {:noreply, state}

      {:needs_migration, migrations} ->
        Logger.info("Needs migration on shard #{state.shard_id}")
        DB.Migrator.migrate(state.conn, migrations)
        {:noreply, state}

      :needs_write_access ->
        Logger.info("Migrating shard #{state.shard_id} via write connection")
        DB.begin(state.context, state.shard_id, :write)
        DB.commit()
        {:noreply, state}
    end
  end
  ...
  def handle_call({:begin, txn_type}, _from, %{transaction_id: nil} = state) do
    {sql, _, _} = DB.Query.fetch!({:begin, txn_type})

    case DB.SQLite.exec(state.conn, sql) do
      :ok ->
  ...
  def handle_call(
        {:query, type, {domain, query_name}, bindings_values},
        _from,
        state
      ) do
    query_id = {state.context, domain, query_name}
    {sql, _, _} = query = DB.Query.fetch!(query_id)
  ...
  def handle_call({:raw, raw_sql, bindings}, _from, state) do
    ...
    sql = DB.Query.inline_bind(raw_sql, bindings)
  ...
  defp create_schema_from_rows(
         query_id,
         {_, {fields_bindings, _}, :select},
         rows
       ) do
    model = get_model_from_query_id(query_id)

    Enum.map(rows, fn row -> DB.Schema.from_row(model, fields_bindings, row) end)
  end

  defp create_schema_from_rows(
         query_id,
         {_, {_, params_bindings}, :insert},
         rows
       ) do
    model = get_model_from_query_id(query_id)

    Enum.map(rows, fn row -> DB.Schema.from_row(model, params_bindings, row) end)
  end
  ...
  defp format_custom(:all, schema, rows) do
    Enum.map(rows, fn row -> DB.Schema.from_row(schema, [:*], row) end)
  end
