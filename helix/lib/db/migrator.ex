  ...
  defp apply_migration!(conn, migrations, domain, v) do
    ...
    case migration do
      {:sql_only, sql_file} ->
        ...
        |> Enum.each(fn query -> DB.SQLite.raw!(conn, query) end)
  ...
  def migrations_dir do
    cond do
      test_dir = Process.get(:db_migrations_dir, false) ->
        if Mix.env() == :prod, do: raise("Unexpected migrations dir")
  ...
  def get_latest_version(domain) do
    if @env == :test do
      if Process.get({:migrator, :latest_version, domain}, false) do
        Process.get({:migrator, :latest_version, domain})
      else
        :persistent_term.get({:migrator, :latest_version, domain})
      end
    else
      :persistent_term.get({:migrator, :latest_version, domain})
    end
