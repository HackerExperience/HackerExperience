defmodule DB.Query do
  ...
  """
  def compile_adhoc_query(
        {context, domain, query_name} = query_id,
        custom_fields
      ) do
  ...
  def get_templated_query_id(
        {_context, _domain, query_name} = query_id,
        target_fields,
        _meta
      )
  ...
  defp append_runtime_query(
         {context, domain, query_name},
         {_, _, _} = adhoc_query
       ) do
    if @env == :test,
      do:
        :persistent_term.put(
          :last_db_sql_query_appended_at,
  ...
  def fetch!({:pragma, :set_user_version}),
    do: {"PRAGMA user_version = ?", [], nil}
  ...
  defp handle_line(line, qs, id, {sql, b, qt})
       when not is_nil(id) and not is_nil(qt),
  ...
  defp handle_atstring(
         <<98, 105, 110, 100, 32, 91, raw_bindings::binary>>,
         qs,
         id,
         q
       ) do
