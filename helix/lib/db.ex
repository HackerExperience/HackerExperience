defmodule DB do
  @moduledoc """
  NOTE: When calling DB functions, we assume a previous setup step has been
  ...
  @type context :: :lobby | :universe | :player | :server
  ...
  def one({domain, :fetch}, bindings) do
    {get_context!(), domain, :__fetch}
    |> DB.Query.get_templated_query_id([], %{})
    |> one(bindings)
  end

  def one({domain, query_name}, bindings) do
    one({get_context!(), domain, query_name}, bindings)
  end

  def one({_, domain, query_name}, bindings) do
    case GenServer.call(
           get_pid!(),
           {:query, :one, {domain, query_name}, bindings}
         ) do
      {:ok, r} -> r
  ...
  def all(schema, _bindings) when is_atom(schema) do
    ...
    |> DB.Query.get_templated_query_id(:all, %{})
    |> all([])
  end
  ...
  def all({_, domain, query_name}, bindings) do
    case GenServer.call(
           get_pid!(),
           {:query, :all, {domain, query_name}, bindings}
         ) do
      {:ok, rows} -> rows
  ...
  def insert(%schema{} = struct) do
    ...
    |> DB.Query.get_templated_query_id(:all, %{schema: schema})
    |> insert(struct)
  end
  ...
  def insert({_, domain, query_name} = full_query_id, %_{} = struct) do
    ...
    GenServer.call(
      get_pid!(),
      {:query, :insert, {domain, query_name}, bindings}
    )
  end
  ...
  def update(%schema{} = struct) do
    ...
    |> DB.Query.get_templated_query_id(struct.__meta__.target, %{})
    |> update(struct)
  end
  ...
  def update({_, domain, query_name} = full_query_id, %_{} = struct) do
    ...
    GenServer.call(
      get_pid!(),
      {:query, :update, {domain, query_name}, bindings}
    )
  end
  ...
  def delete({_, domain, query_name} = full_query_id, %_{} = struct) do
    ...
    GenServer.call(
      get_pid!(),
      {:query, :delete, {domain, query_name}, bindings}
    )
  end
  ...
  defp get_bindings(query_id, struct) do
    {_, {_, params_bindings}, _} = DB.Query.fetch!(query_id)

    # IO.puts "params bindings"
    # IO.inspect(params_bindings)

    # IO.inspect(DB.Query.fetch!(query_id))
    ...
    Enum.map(params_bindings, fn field_name ->
      DB.Schema.dump(struct, field_name)
    end)
  end
