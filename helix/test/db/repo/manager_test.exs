defmodule DB.Repo.ManagerTest do
  ...
  setup %{shard_id: shard_id, db: _db} = flags do
    manager_pid =
      unless Map.get(flags, :skip_init, false) do
        {:ok, pid} = start_supervised({Manager, {@context, shard_id}})
        pid
      else
        nil
      end
  ...
  describe "create/1" do
    ...
    test "starts a supervised Manager", %{shard_id: shard_id} do
      {:ok, pid} = Manager.create(@context, shard_id)
      state = :sys.get_state(pid)

      assert state.shard_id == shard_id
      refute state.write_1.pid
      refute state.write_1.busy?
      refute state.read_1.pid
      refute state.read_1.busy?
    end
  end
  ...
  describe "fetch_connection/2 - write" do
    test "creates the first connection", %{manager: manager} do
      ...
      assert repo_state.path =~ @data_dir
  ...
  describe "close_connection/2" do
    test "closes the connection", %{manager: manager} = ctx do
      Test.DB.ensure_migrated(:test, ctx.shard_id)
