defmodule DB.Migrator.MetadataTest do
  ...
  describe "insert_migration/3" do
    ...
    test "inserts migration", %{conn: conn} do
      ...
      assert [["core", 2, _], ["mob", 1, _]] =
               SQLite.raw!(
                 conn,
                 "SELECT * FROM #{@summary_table} ORDER BY domain ASC"
               )
    end
  end
  ...
  describe "summarize_migrations/2" do
    ...
    test "returns the latest version for each domain", %{conn: conn} do
      ...
      assert %{core: 4, mob: 2, inventory: 1, test: 0} ==
               Metadata.summarize_migrations(conn, :test)
    end
  end
end
