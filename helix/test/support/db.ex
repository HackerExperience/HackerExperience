defmodule Test.DB do
  ...
  def on_start do
    ...
    Enum.each(@contexts, fn context ->
      File.mkdir_p!("#{test_dbs_path()}/#{context}")
    end)
  ...
  def ensure_migrated(context, shard_id) do
    ...
    {:ok, pid} = DB.Repo.start_link({context, shard_id, path, :readwrite})
  ...
  def random_autoincrement(schema) do
    ...
    DB.raw!("delete from #{table} where id = #{rand}")
  end
